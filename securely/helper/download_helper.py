...
class DownloadHelper(object):
    ...
    def fetch_files_for_user_in_trusted_area(self,latitude,longitude,user_id):
        try:
            ...
            for file_details in list_of_files_by_user:
                file_latitude = self.decrypt_coords(file_details._latitude, file_details._IV, file_details._key)
                file_longitude = self.decrypt_coords(file_details._longitude, file_details._IV, file_details._key)
                file_loc_dict = dict()
                file_loc_dict['latitude'] = file_latitude
                file_loc_dict['longitude'] = file_longitude
                distance = self.calculate_distance(loc_dict,file_loc_dict)
                if distance <= 3:
                    rest_dict = dict()
